...
def main():
    ...
    upa_graph = App2.make_upa_graph(App2_1.INITIAL_NODES, num_nodes)
    xvals = range(num_nodes+1)
    yvals_network = proj2.compute_resilience(network_graph, App2_3.fast_targeted_order(network_graph))
    yvals_er = proj2.compute_resilience(er_graph, App2_3.fast_targeted_order(er_graph))
    yvals_upa = proj2.compute_resilience(upa_graph, App2_3.fast_targeted_order(upa_graph))
